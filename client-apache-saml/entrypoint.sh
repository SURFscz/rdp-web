...
if [ -d /opt/data/metadata ]; then
  ...
  wget -O "/opt/idp.xml" "$idp_metadata"
  sed -i -e "/# Metadata conf/a MellonIdPMetadataFile /opt/idp.xml" $SAML_CONF
fi
